...
function Get-QrPenalty {
    ...
    for ([int]$y = 0; $y -lt $size; $y++) {
        [int]$runColor = $Matrix[0, $y]
        [int]$runLen = 1
        for ([int]$x = 1; $x -lt $size; $x++) {
            if ($Matrix[$x, $y] -eq $runColor) { $runLen++ }
            else {
                if ($runLen -ge 5) { $penalty = $penalty + 3 + ($runLen - 5) }
                $runColor = $Matrix[$x, $y]; $runLen = 1
            }
        }
        if ($runLen -ge 5) { $penalty = $penalty + 3 + ($runLen - 5) }
    }
    for ([int]$x = 0; $x -lt $size; $x++) {
        [int]$runColor = $Matrix[$x, 0]
        [int]$runLen = 1
        for ([int]$y = 1; $y -lt $size; $y++) {
            if ($Matrix[$x, $y] -eq $runColor) { $runLen++ }
            else {
                if ($runLen -ge 5) { $penalty = $penalty + 3 + ($runLen - 5) }
                $runColor = $Matrix[$x, $y]; $runLen = 1
    ...
    for ([int]$y = 0; $y -lt ($size - 1); $y++) {
        for ([int]$x = 0; $x -lt ($size - 1); $x++) {
            [int]$c = $Matrix[$x, $y]
            [int]$x1 = $x + 1
            [int]$y1 = $y + 1
            if ($Matrix[$x1, $y] -eq $c -and $Matrix[$x, $y1] -eq $c -and $Matrix[$x1, $y1] -eq $c) {
                $penalty = $penalty + 3
            }
        }
    }
    ...
    [int]$total = $size * $size
    $penalty = $penalty + [Math]::Floor([Math]::Abs($dark * 20 - $total * 10) / $total) * 10
    return $penalty
}
